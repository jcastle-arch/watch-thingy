...
function Ticker({ catalog }) {
  const items = useMemo(() => {
    ...
    return [...arr, ...arr, ...arr]; // triple for seamless loop
  }, [catalog]);
  return (
...
// ── Sidebar / watchlist ────────────────────────────────────────
function Sidebar({ catalog, activeId, onPick, query, setQuery, onAddAlert }) {
...
function HeroPanel({ watch, summary, verdict: v, loading }) {
  ...
  const placePct = (v) => (v - rangeLo) / (rangeHi - rangeLo) * 100;
...
function RetailerTable({ retailers, quotes, summary, watch, includedIds, toggle, loading }) {
  // Find min for "best price" highlight
    ...
    <div className="col retailers-col">
      ...
      <div className="r-rows">
        {quotes.map((q, i) => {
          ...
          // Bar fill width relative to overall range
          let barPct = 50;
          let barClass = '';
          ...
          return (
            <div
              ...
              <div className="nm">
                <span className="n">
                  {r.kind === 'OEM' ? window.WPX_DATA.BRAND_SITES[watch.brand] || watch.brand + '.com' : r.name}
...
function HistoryChart({ history, summary, watch, range, onRange, marketNote }) {
  ...
  const area = `${path('avg').replace('M', 'M')} L ${x(slice.length - 1).toFixed(1)} ${(padT + h).toFixed(1)} L ${padL.toFixed(1)} ${(padT + h).toFixed(1)} Z`;
    ...
    <div className="chart-wrap">
      <div className="chart-head">
        ...
        <div className="now">
          ...
          <div className="pair"><span className="k">Period Δ</span>
